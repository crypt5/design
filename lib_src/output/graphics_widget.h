...
typedef void(*onclick)(GUI* g, WIDGET* w);
typedef void(*onselect)(GUI* g, WIDGET* w);
struct graphics_widget_t{
  ...
  onclick click;//Function to paint when widget is clicked
  onselect select;//Functio to print widget when selected
  char* string;
  void* data;
};
