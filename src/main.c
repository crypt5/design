...
int main()
{
    ...
    log = logger_init("logs/Program_Output.log", 0, 100000);
...
// If the code is running on the micro controller
    ...
    mod1 = setup_actuator_module(enable1, dir1, step1, far1, near1, AIN1,
            offset1, slope1);
    mod2 = setup_actuator_module(enable2, dir2, step2, far2, near2, AIN2,
            offset2, slope2);

    ex_force = setup_extern_force_device(
            config_get_int(config, "externalforceSensorAIN"));
...
external grip force (N),external grip displacement (mm)");
            ...
            sprintf(buf,
                    "%0.4lf,%0.4lf,%0.4lf,%0.4lf,%0.4lf,%0.4lf,%0.4lf,%0.4lf,",
                    time, mot1f, mot1d, mot2f, mot2d, ef, egf, egd);
